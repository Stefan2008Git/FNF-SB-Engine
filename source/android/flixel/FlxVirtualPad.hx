	...
	public function new(?DPad:FlxDPadMode, ?Action:FlxActionMode, ?alphaAlt:Float = 0.75, ?antialiasingAlt:Bool = true) {
		...
		if (ClientPrefs.data.gameStyle == 'SB Engine') {
			...
			switch (Action) {
				...
				case NOTESCOLORMENU:
					...
					actions.add(add(buttonB = createButton(0, FlxG.height - 235 * 3, 44 * 3, 127, "b", FlxColor.BROWN)));					
				case NONE:
			}
		} else {
			switch (DPad){
		...
		switch (Action){
			...
			case NOTESCOLORMENU:
				...
				actions.add(add(buttonB = createButton(0, FlxG.height - 235 * 3, 44 * 3, 127, "b", 0xFFCB00)));					
			case NONE:
			}
		}
	}	
